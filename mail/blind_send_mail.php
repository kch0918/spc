...
function blind_send_mail($id)
{
    ...
    $query = "select * ,DATE_FORMAT(submit_date, '%Y-%m-%d') as submit_date2 from SPC_NAGATION where id='$id'";
    ...
    $toEmail = "kwanpyo@spc.co.kr";
    // $toEmail = "mdh0088@musign.net";
    ...
    $message .= "<tr style='border-bottom:1px solid #eaeaea;'>
    						...
    						<td style='color:#7d7d7d; font-size:16px; padding: 20px 0 20px 30px; text-align: left; border-bottom:1px solid #eaeaea; letter-spacing: -0.05em; font-weight: 600;'>익명</td>
    ...
    $message .= "  <tr style='border-bottom:1px solid #eaeaea;'>
					   <th class='top-th' style='background:#f8f8f8; color:#afafaf; font-size:16px; padding: 23px 0 23px 0; border-bottom:1px solid #eaeaea; border-right:1px solid #eaeaea; letter-spacing: -0.05em; font-weight:800;'>첨부파일</th>
					   <td colspan='3' style='color:#7d7d7d; font-size:16px; padding: 20px 0 20px 30px; text-align: left; border-bottom:1px solid #eaeaea; letter-spacing: -0.05em; font-weight: 600;'>
                        <button type='button'><a download href='http://www.spc.co.kr/aDmin/file/{$user_file}' >{$user_file}</a></button>
					   </td>
				    </tr>
				</table>
			</div>
    ...
    try {
        ...
        $mail->addcc("yangseog.seo@spc.co.kr", "SPC담당자"); // 2021-12-15 동동이 수정
        $mail->addcc("catchu79@spc.co.kr", "SPC담당자"); // 2021-12-15 동동이 수정
        $mail->addcc("torsokang@spc.co.kr", "SPC담당자"); // 2021-12-15 동동이 수정 */
         // 숨은참조
    /*   $mail -> addBcc("coscoswkd@musign.net", "SPC담당자"); // 2021-12-15 동동이 수정

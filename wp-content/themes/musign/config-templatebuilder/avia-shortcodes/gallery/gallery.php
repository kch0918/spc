			...
			if( ! empty( $attachments ) && is_array( $attachments ) )
			{
				...
				foreach( $attachments as $attachment )
				{
					...
					if( strpos( $style, 'big_thumb' ) !== false && $first )
					{
						...
						$img_tag = Av_Responsive_Images()->add_attachment_id( $img_tag, $attachment->ID );
					...
					$thumbs .= " <a href='{$link[0]}' data-rel='gallery-" . self::$gallery . "' data-prev-img='{$prev[0]}' {$class} data-onclick='{$counter}' title='{$description}' {$markup_url}>{$img_tag}</a>";
					$first = false;
				}

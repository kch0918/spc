...
function main_news_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/latest.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_news', 'main_news_shortcode');

function main_news_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/latest_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_news_en', 'main_news_en_shortcode');

function main_news_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/latest_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_news_cn', 'main_news_cn_shortcode');

function main_magazine_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/latest.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_magazine', 'main_magazine_shortcode');

function main_magazine_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/latest_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_magazine_en', 'main_magazine_en_shortcode');

function main_brand_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_brand.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_brand', 'main_brand_shortcode');

function main_brand_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_brand_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_brand_en', 'main_brand_en_shortcode');

function main_brand_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_brand_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_brand_cn', 'main_brand_cn_shortcode');

function main_insta_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_insta.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_insta', 'main_insta_shortcode');

function brandList_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/brandList.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('brandList', 'brandList_shortcode');

function brandList_en_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/brandList_en.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('brandList_en', 'brandList_en_shortcode');

function brandList_cn_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/brandList_cn.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('brandList_cn', 'brandList_cn_shortcode');

function brandDetail_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/brandDetail.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('brandDetail', 'brandDetail_shortcode');

function brandDetail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/business/brandDetail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('brandDetail_en', 'brandDetail_en_shortcode');

function brandDetail_cn_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/brandDetail_cn.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('brandDetail_cn', 'brandDetail_cn_shortcode');

function subsidiaryList_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/subsidiaryList.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryList', 'subsidiaryList_shortcode');

function subsidiaryList_en_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/subsidiaryList_en.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryList_en', 'subsidiaryList_en_shortcode');

function subsidiaryList_cn_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/subsidiaryList_cn.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryList_cn', 'subsidiaryList_cn_shortcode');


function subsidiaryDetail_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/subsidiaryDetail.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryDetail', 'subsidiaryDetail_shortcode');

function subsidiaryDetail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/business/subsidiaryDetail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryDetail_en', 'subsidiaryDetail_en_shortcode');

function subsidiaryDetail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/business/subsidiaryDetail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('subsidiaryDetail_cn', 'subsidiaryDetail_cn_shortcode');

function map_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/map.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
...
function map_en_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/map_en.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('map_en', 'map_en_shortcode');

function map_cn_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/business/map_cn.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('map_cn', 'map_cn_shortcode');

function contribution_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/csr/contribution.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('contribution', 'contribution_shortcode');

function contribution_en_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/csr/contribution_en.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('contribution_en', 'contribution_en_shortcode');

function contribution_cn_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/csr/contribution_cn.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('contribution_cn', 'contribution_cn_shortcode');

function finance_report_list_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/board/finance-report/list.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('finance_report_list', 'finance_report_list_shortcode');

function finance_report_list_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/finance-report/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('finance_report_list_en', 'finance_report_list_en_shortcode');

function finance_report_list_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/finance-report/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('finance_report_list_cn', 'finance_report_list_cn_shortcode');

function finance_report_view_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/board/finance-report/view.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('finance_report_view', 'finance_report_view_shortcode');

function notice_list_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_list', 'notice_list_shortcode');

function notice_list_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_list_en', 'notice_list_en_shortcode');

function notice_list_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_list_cn', 'notice_list_cn_shortcode');


function notice_list_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_list_detail', 'notice_list_detail_shortcode');

function notice_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_detail_en', 'notice_detail_en_shortcode');

function notice_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/notice/list_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('notice_detail_cn', 'notice_detail_cn_shortcode');




function youtubeMain_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_youtube.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('youtubeMain', 'youtubeMain_shortcode');

function youtubeMain_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_youtube_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('youtubeMain_en', 'youtubeMain_en_shortcode');

function youtubeMain_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_youtube_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('youtubeMain_cn', 'youtubeMain_cn_shortcode');

function magazineList_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazineList', 'magazineList_shortcode');

function magazineList_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazineList_en', 'magazineList_en_shortcode');

function magazineList_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazineList_cn', 'magazineList_cn_shortcode');

function newsletter_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/newsletter/list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
...
function newsletter_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/newsletter/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsletter_en', 'newsletter_en_shortcode');

function newsletter_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/newsletter/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsletter_cn', 'newsletter_cn_shortcode');

function csrReview_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/review/list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('csrReview', 'csrReview_shortcode');

function csrReview_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/review/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('csrReview_en', 'csrReview_en_shortcode');

function csrReview_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/review/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('csrReview_cn', 'csrReview_cn_shortcode');


function award_performance_list_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/board/award-performance/list.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('award_performance_list', 'award_performance_list_shortcode');


function contribution_detail_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/csr/detail.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('contribution_detail', 'contribution_detail_shortcode');

function contribution_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/csr/detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('contribution_detail_en', 'contribution_detail_en_shortcode');

function contribution_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/csr/detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('contribution_detail_cn', 'contribution_detail_cn_shortcode');

function newsletter_detail_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/board/newsletter/list_detail.php');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('newsletter_detail', 'newsletter_detail_shortcode');

function newsletter_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/newsletter/list_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsletter_detail_en', 'newsletter_detail_en_shortcode');

function newsletter_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/newsletter/list_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsletter_detail_cn', 'newsletter_detail_cn_shortcode');

//공지사항 detail
function notice_detail_shortcode($atts) {
// turn on output buffering to capture script output
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'../../../../front/board/notice/list_detail.php');
...
//register the Shortcode handler
add_shortcode('notice_detail', 'notice_detail_shortcode');

//재단보고 detail
function finance_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/finance-report/list_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('finance_detail', 'finance_detail_shortcode');

function finance_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/finance-report/list_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('finance_detail_en', 'finance_detail_en_shortcode');

function finance_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/finance-report/list_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('finance_detail_cn', 'finance_detail_cn_shortcode');


//뉴스 리스트
function newsfeed_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
...
function newsfeed_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsfeed_en', 'newsfeed_en_shortcode');

function newsfeed_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('newsfeed_cn', 'newsfeed_cn_shortcode');


//review detail
function Review_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/review/list_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('Review_detail', 'Review_detail_shortcode');

//snschannel
function snschannel_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/snschannel.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('snschannel', 'snschannel_shortcode');

//snschannel_en
function snschannel_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/snschannel_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('snschannel_en', 'snschannel_en_shortcode');

//snschannel_cn
function snschannel_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/snschannel_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('snschannel_cn', 'snschannel_cn_shortcode');

//cfchannel
function CFchannel_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/CFchannel.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('CFchannel', 'CFchannel_shortcode');

function CFchannel_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/CFchannel_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('CFchannel_en', 'CFchannel_en_shortcode');

function CFchannel_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/CFchannel_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('CFchannel_cn', 'CFchannel_cn_shortcode');

//youtubechannel
function youtubechannel_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/youtubechannel.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('youtubechannel', 'youtubechannel_shortcode');

//familychannel
function channel_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/channel.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('channel', 'channel_shortcode');

function channel_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/channel_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('channel_en', 'channel_en_shortcode');

function channel_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/media/channel_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('channel_cn', 'channel_cn_shortcode');

//news detail
function news_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('news_detail', 'news_detail_shortcode');

//news detail
function news_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('news_detail_en', 'news_detail_en_shortcode');

//news detail
function news_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/news/list_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('news_detail_cn', 'news_detail_cn_shortcode');

//magazine detail
function magazine_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazine_detail', 'magazine_detail_shortcode');

//magazine detail
function magazine_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazine_detail_en', 'magazine_detail_en_shortcode');

//magazine detail
function magazine_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/board/magazine/list_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('magazine_detail_cn', 'magazine_detail_cn_shortcode');

//magazine detail
function brandCate_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/news/brandCate.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('brandCate', 'brandCate_shortcode');

//정도경영 부정제보 인트로
function tipOff_intro_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/intro.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_intro', 'tipOff_intro_shortcode');

function tipOff_intro_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/intro_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_intro_en', 'tipOff_intro_en_shortcode');

function tipOff_intro_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/intro_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_intro_cn', 'tipOff_intro_cn_shortcode');

//정도경영 부정제보 실명
function tipOff_name_report_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/name_report.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_name_report', 'tipOff_name_report_shortcode');

function tipOff_name_report_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/name_report_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_name_report_en', 'tipOff_name_report_en_shortcode');

function tipOff_name_report_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/name_report_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_name_report_cn', 'tipOff_name_report_cn_shortcode');

//정도경영 부정제보 익명
function tipOff_blind_report_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/blind_report.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_blind_report', 'tipOff_blind_report_shortcode');

function tipOff_blind_report_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/blind_report_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_blind_report_en', 'tipOff_blind_report_en_shortcode');

function tipOff_blind_report_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/blind_report_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_blind_report_cn', 'tipOff_blind_report_cn_shortcode');

//정도경영 부정제보 완료
function tipOff_complete_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/complete.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_complete', 'tipOff_complete_shortcode');

function tipOff_complete_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/complete_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_complete_en', 'tipOff_complete_en_shortcode');

function tipOff_complete_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/complete_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_complete_cn', 'tipOff_complete_cn_shortcode');

//정도경영 부정제보 로그인
function tipOff_view_login_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_login.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_login', 'tipOff_view_login_shortcode');

function tipOff_view_login_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_login_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_login_en', 'tipOff_view_login_en_shortcode');

function tipOff_view_login_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_login_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_login_cn', 'tipOff_view_login_cn_shortcode');

//정도경영 부정제보 리스트
function tipOff_view_list_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list', 'tipOff_view_list_shortcode');

function tipOff_view_list_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list_en', 'tipOff_view_list_en_shortcode');

function tipOff_view_list_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list_cn', 'tipOff_view_list_cn_shortcode');

//정도경영 부정제보 상세
function tipOff_view_detail_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail', 'tipOff_view_detail_shortcode');

function tipOff_view_detail_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail_en', 'tipOff_view_detail_en_shortcode');

function tipOff_view_detail_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail_cn', 'tipOff_view_detail_cn_shortcode');

//정도경영 부정제보 목록 - 익명
function tipOff_view_list_blind_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list_blind.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list_blind', 'tipOff_view_list_blind_shortcode');

function tipOff_view_list_blind_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list_blind_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list_blind_en', 'tipOff_view_list_blind_en_shortcode');

function tipOff_view_list_blind_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_list_blind_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_list_blind_cn', 'tipOff_view_list_blind_cn_shortcode');


//메인 해피포인트
function main_point_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_point.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_point', 'main_point_shortcode');


function main_point_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/main/main_point_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('main_point_en', 'main_point_en_shortcode');
// 추가 메뉴 관리
...
function custom_menu() {
    echo '<style>.notice, div.error, div.updated{margin-left:2px;} a.admbtn{padding:10px 20px; display:inline-block; text-decoration:none; color:#fff; background:#333; margin-right:10px;}</style> <br> <a href="/aDmin/popup/popup_list.php" target="winblank" class="admbtn">팝업 관리</a>';
}
...
// Disable access to author page 작성자 페이지에 대한 액세스를 비활성화
add_action('template_redirect', 'my_custom_disable_author_page');

//정도경영 부정제보 상세(익명)
function tipOff_view_detail_blind_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail_blind.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail_blind', 'tipOff_view_detail_blind_shortcode');

function tipOff_view_detail_blind_en_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail_blind_en.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail_blind_en', 'tipOff_view_detail_blind_en_shortcode');

function tipOff_view_detail_blind_cn_shortcode($atts) {
    // turn on output buffering to capture script output
    ob_start();
    // include file (contents will get saved in output buffer)
    include(TEMPLATEPATH.'../../../../front/tip-off/view_detail_blind_cn.php');
    // save and return the content that has been output
    $content = ob_get_clean();
    return $content;
}
//register the Shortcode handler
add_shortcode('tipOff_view_detail_blind_cn', 'tipOff_view_detail_blind_cn_shortcode');
...
ob_start();
// include file (contents will get saved in output buffer)
include(TEMPLATEPATH.'/include/main.html');
// save and return the content that has been output
$content = ob_get_clean();
return $content;
}
//register the Shortcode handler
add_shortcode('main', 'main_shortcode');
*/
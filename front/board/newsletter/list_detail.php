...
$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_NEWS where idx = {$_REQUEST['idx']}";
// print_r($_POST);
...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_NEWS where lang = 'ko' and idx < {$_REQUEST['idx']} order BY idx desc limit 1";//쿼리문
$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_NEWS where lang = 'ko' and idx > {$_REQUEST['idx']} limit 1";//쿼리문
...
<div class="board-view">
			...
			if ($row_before_musign['idx']!=null) {
				...
				<div>
    				<div class="table">
    					...
    					<div class="bview-ltit"><a href="javascript:goNews(<?php echo $row_before_musign['idx']?>);">
			...
			if ($row_after_musign['idx']!=null) {
    			...
    			<div>
    				<div class="table">
    					...
    					<div class="bview-ltit"><a href="javascript:goNews(<?php echo $row_after_musign['idx']?>);">
	...
	<div class="btnwr img-ani bottom-top">
		<a href="/csr/society/newsletter/" class="main-btn home-href">목록</a>
	</div>
</div>
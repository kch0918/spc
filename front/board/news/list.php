...
$query2 = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_REPORT where expo_yn='Y' and lang='ko'";
// print_r($_POST);
...
{
	location.href="/media-hub/news/detail/?seq="+seq;
}
	...
	$.ajax({
		...
		success: function(data){
    			...
    			for(var i = 0; i < result.length; i++) {
    	   			...
    	   			inner += 	'<li><a href="javascript:cateType(\''+result[i].cate_name+'\');" value="'+result[i].cate_name+'">'+result[i].cate_name+'</li>';
	...
	$.ajax({
		...
		success: function(data){
    			...
    			for(var i = 0; i < result.length; i++) {
    				...
    				inner += 	'<li><a href="javascript:cateType(\''+result[i].cate_name+'\');" name="cateType">'+result[i].cate_name+'</li>';
...
<div class="contri-wr text-center">
	<div class="contri-sec contri-sec01">
		...
		<form id="fncForm" name="fncForm" method="post" action="/media-hub/news/">
			...
			<div class="contri-list cardtype-list cardtype-list02 slide-wr">
				<div class="swiper-container">
					<ul class="swiper-wrapper tranDelayList">
						<?php 
                        $query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_REPORT where expo_yn = 'Y' and lang = 'ko' order by submit_date DESC LIMIT 5 ";
						...
						<li class="swiper-slide">
							...
							<?php 
								...
								<div class="cont">
									...
									?>
										<span class="writer">SPC 그룹</span><time class="date"><?php echo $row['submit_date2']?></time>
									<?php 
									}
									?>
	...
	<div class="contri-sec contri-sec04">
		<h2 class="ft-42 bold color-000 global-tit img-ani bottom-top">LIVE 보도자료</h2>
			...
			<div class="board-search table">
				<div class="contri-data">
					<div class="table">
						<div>
							<div class="select">
								<p id="type_name"><strong>전체</strong></p>
								<ul>
									<li><a href="javascript:allCate();">전체</a></li>
									<li><a href="javascript:brandCate('brand');">브랜드</a></li>
									<li><a href="javascript:subCate('subsidiary');">계열사</a></li>
									<li><a href="javascript:groupCate('spc_group');">SPC 그룹</a></li>
								</ul>
							</div>
						</div>
						<div>
							<div class="select">
								<p id="type_name2"><strong>전체</strong></p>
    								<ul id="Cate2">
    								</ul>
							</div>
						</div>

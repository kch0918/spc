...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_REPORT where seq < {$_REQUEST['seq']} and lang = 'cn' order by seq desc limit 1";//쿼리문
$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_REPORT where seq > {$_REQUEST['seq']} and lang = 'cn' limit 1";//쿼리문
...
{
    location.href="/media-hub/news/detail_cn/?seq="+seq + "&lang=zh-hans";
}
...
<div class="board-view">
	...
	<div class="btnwr img-ani bottom-top">
		<a href="/media-hub/news/?lang=zh-hans" class="main-btn home-href">目录</a>
	</div>
</div>
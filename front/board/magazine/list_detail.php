...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language='국문' and idx < {$_REQUEST['idx']} order BY idx desc limit 1";//쿼리문
$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language='국문' and idx > {$_REQUEST['idx']} limit 1";//쿼리문
					...
					<div class="slide-wr">
						<div class="magazine-latest swiper-container">
							<ul class="tranDelayList swiper-wrapper">
								<?php 
								$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where expo_yn='Y' order by submit_date DESC LIMIT 5";
	...
	<div class="maga-detail">
		...
		<div class="maga-detop">
			...
			<p><strong><?php echo $row2['title']?></strong></p>
				<ul>
				<?php 
					$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where expo_yn='Y' order by submit_date DESC";
					$result = sql_query($query);
					for($i = 0; $i < sql_count($result); $i++){
					$row = sql_fetch($result);
				?>

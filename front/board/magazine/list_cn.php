...
$query2 = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language = '중문' ORDER BY submit_date DESC LIMIT 1";
// print_r($_POST);
...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language = '중문' and idx < 1 order BY idx desc limit 1";//쿼리문
$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language = '중문' and idx > 76 limit 1";//쿼리문
...
{
	location.href="/media-hub/magazine/detail_cn/?idx="+idx+ "&lang=zh-hans";
}
					...
					<div class="slide-wr">
						<div class="magazine-latest swiper-container">
							<ul class="tranDelayList swiper-wrapper">
								<?php 
								$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where expo_yn='Y' and language = '중문' order by submit_date DESC LIMIT 5";
	...
	<div class="maga-detail">
		...
		<div class="maga-detop">
			<div class="select">
				...
				<ul>
				<?php 
					$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where expo_yn='Y' and language = '중문' order by submit_date DESC";
					$result = sql_query($query);
					for($i = 0; $i < sql_count($result); $i++){
					$row = sql_fetch($result);
				?>
			...
			<div>
				<div class="table">
					<div>
						...
						<div class="btn-wr">
							<a href="http://spcweb.musign.co.kr/media-hub/magazine/?lang=zh-hans" class="btn btn01">취소하기</a>

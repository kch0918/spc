...
$query2 = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where language = '영문' order by submit_date DESC LIMIT 1";
// print_r($_POST);
...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where idx < 1 and language = '영문' order by idx desc limit 1";//쿼리문

$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_MAGAZINE where idx > 73 and language = '영문' limit 1";//쿼리문
			...
			<div>
				<div class="table">
					<div>
						...
						<div class="btn-wr">
							<a href="http://spcweb.musign.co.kr/media-hub/magazine/?lang=en" class="btn btn01">취소하기</a>

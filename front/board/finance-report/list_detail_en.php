...
//이전 글
$query_before_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_FINANCE where idx < {$_REQUEST['idx']} and lang = 'en' order BY idx desc limit 1";//쿼리문
$result_before_musign =  sql_query($query_before_musign); //쿼리실행
$row_before_musign = sql_fetch($result_before_musign);

//다음 글
$query_after_musign = "select * ,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_FINANCE where idx > {$_REQUEST['idx']} and lang = 'en' limit 1";//쿼리문
...
{
    location.href="/share/spc-foundation/finance-report-list/detail_en/?idx="+idx + "&lang=en";
}
...
<div class="board-view">
	...
	<div class="btnwr img-ani bottom-top">
		<a href="/csr/spc-foundation/finance-report-list/?lang=en" class="main-btn home-href">List</a>
	</div>
</div>
...
if ($_SESSION['user_idx'] == "") {
    ...
    echo "<meta http-equiv='refresh' content='0;url=/csr/right-mng/tip-off-login/?lang=zh-hans'>";
    
    exit;
}


$query  = "SELECT *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_NAGATION where name = '{$_SESSION['user_name']}'";
...
function goDetail(idx) {
	location.href="/csr/right-mng/view-detail/?lang=zh-hans&idx="+idx;
}
</script>
...
<div class="report-list">
    <div class="board-default-list">
    	...
    	<form id="fncForm" name="fncForm" method="post" action="/csr/right-mng/view-list/">
            <table>
                <caption>我的举报结果</caption>
                <colgroup>
                	<col>
                	<col>
                	<col>
            	...
            	<?php 
            		...
            		<tr>
                		...
                		<td class="subject"><a href="javascript:goDetail('<?php echo $row['idx']?>')"><?php echo $row['title']?></a></td><!-- 게시글 내용으로 들어가는 링크 -->
                		<td><?php echo $row['submit_date2']?></td>
            			<?php 
            			if($row['status'] == "Y") {
            			    ?>
                    		<td><span class="status complete">답변완료</span></td><!-- 진행상태가 매 번 컬러 다를 것 같아서 상태값은 영문으로 된 걸 클래스에 같이 넣어주세요!! !!!! -->
            			<?php 
            			} else {
            		    ?>
    ...
    <div class="page_box">
        <div class="board-pagination">
        		<a href="" class="prev_ prev-btn"  onclick="pageMove(<?=$s_page-1?>)">以前</a>
                <?php 
                $pagingCnt = 0;
                if($e_page != 0)
                {
                ...
                if($pageNum != $page && $pageNum > 5)
                {
                    if($pagingCnt > 4)
                    {
                        ...
                        {
                            ?>
                            <a class="next-btn next_" onclick="pageMove(<?=$e_page+1?>)">以後</a>
                            <?php
                        }
                    }
                }
        		...
        		<?php 
    				if($pagingCnt < 4){
    				?>
    				 <a href="" class="next-btn next_"  onclick="pageMove(<?=$e_page+1?>)">以後</a>
    				<?php 
                    }      
			    ?>
        </div>

...
function goSub(idx)
{
	location.href="/aDmin/board/branch_cn_edit.php?idx="+idx;
}
...
function changeFunc() {
	 ...
	 location.href = "/aDmin/board/branch_cn.php?order_by="+selectedValue;
...
<div id="container" class="report">	
	...
	<form id="fncForm" name="fncForm" method="post" action="branch_cn.php">
                        ...
                        for ($i=0; $i<sql_count($result); $i++)
                        {
                            ...
                            ?>
            					<tr class="drag-btn" data-idx="<?php echo $row['idx']?>">
        							<td><?php  echo $i+1;?></td>
        							<td><?php echo $row['cate_brand_cn'];?></td>

...
$query = "SELECT *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 FROM SPC_REVIEW where 1";
...
function changeFunc() {
	 ...
	 location.href = "/aDmin/csr/review.php?order_by="+selectedValue;
...
{
	location.href="/aDmin/csr/review_edit.php?idx="+idx;
}
...
function goDelete(idx)
{
	if(confirm("해당 게시물을 삭제하시겠습니까?"))
    {
	   ...
	   $.ajax({
   		type: "post",
   		url: "/aDmin/csr/review_delete_proc.php",
   		dataType:"text",
   		data : 
   		{
       		idx : idx
...
<div id="container" class="report magazine">
	<div class="search">
		<p>참여후기<strong>생성된 게시글을 관리합니다.</strong></p>
		<form id="fncForm" name="fncForm" method="post" action="review.php">
                        ...
                        for ($i=0; $i<sql_count($result); $i++)
                        {
                            ...
                            ?>
            					<tr>
        							...
        							<td><a href="javascript:goReview(<?php echo $row['idx']?>);"><?php echo $row['title'];?></a></td>
            						...
            						<?php if($row['expo_yn'] == "Y") {
            						    ...
            						    <?php 
                						  ...
                						  ?>
            						<td><a class="edit" onclick="goReview(<?php echo $row['idx']?>)">수정</a></td>

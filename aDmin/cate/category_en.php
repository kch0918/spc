...
<script>
			...
			success : function(data) 
			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        		    	...
        		    	inner +=  	    '<input type="text" data-idx="'+result[i].idx+'" name="cate_brand_en" value="'+result[i].cate_brand_en+'">';
	...
	function getCate_sub() 
	{
		$.ajax({
			type : "POST", 
			url : "/aDmin/cate/getCate_sub_en.php",
			dataType : "text",
			async : false,
			error : function() 
			{
			...
			success : function(data) 
			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        		    	...
        		    	inner +=  	    '<input type="text" data-sub-idx="'+result[i].idx+'" name="cate_brand_en" value="'+result[i].cate_brand_en+'">';
    ...
    function editcate_proc() {
    	...
    	cate_brand_en = $('input[data-idx='+chkidx+']').val(); 
    	
    	console.log(chkidx);
    	console.log(cate_brand_en);
    	...
    	$.ajax({
    			type : "POST", 
    			url : "/aDmin/cate/editcate_en_proc.php",
    			dataType : "text",
    			async : false,
    			data : 
    			{
    ...
    function editcate_sub_proc() {
    	...
    	cate_brand_en = $('input[data-sub-idx='+chkidx+']').val(); 
    	...
    	$.ajax({
    			type : "POST", 
    			url : "/aDmin/cate/editcate_sub_en_proc.php",
    			dataType : "text",
    			async : false,
    			data : 
    			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
					if(result[i].expo_yn == "Y")
					{
        		    	...
        		    	inner +=            '<td><input type="text" data-idx2='+result[i].idx+' name="brand_name" value="'+result[i].cate_name_en+'"/></td>';
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
    			 	if(result[i].expo_yn == "Y")
    				{
            		    	...
            		    	inner +=            '<td><input type="text" data-sub-idx2='+result[i].idx+' name="brand_name" value="'+result[i].cate_name_en+'"/></td>';
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        				...
        				if(result[i].parentidx == 1) {
                       		inner +=    '<option value="'+result[i].parentidx+'">Bakery & desserts</option>';
        				} else if(result[i].parentidx == 2) {
        					inner +=    '<option value="'+result[i].parentidx+'">Dining</option>';
        				} else if(result[i].parentidx == 3) {
        					inner +=    '<option value="'+result[i].parentidx+'">Coffee & other beverages</option>';
        				} else if(result[i].parentidx == 4){
        					inner +=    '<option value="'+result[i].parentidx+'">Distribution & services</option>';
        				} 
        		    	...
        		    	inner += '<tr>';
        		    	inner += 	'<th>브랜드명</th>';
        		    	inner += 		'<td><input type="text" name="brand_name2" value="'+nullChk(result[i].cate_name_en)+'"/></td>';
        		    	inner +=  '</tr>';
        		    	
        		    	inner +=   '<tr>';
        		    	inner +=     '<th>브랜드 설명</th>';
        		    	inner +=          '<td><input type="text" name="brand_dis" value="'+nullChk(result[i].cate_dis_en)+'"/></td>';
        		    	inner +=   '</tr>';
			 	...
			 	var inner = "";
        			 	...
        			 	inner = '<tr>';
          				...
          				if(result[i].parentidx == 5) {
                         		inner +=    '<option value="'+result[i].parentidx+'">Food</option>';
          				} else if(result[i].parentidx == 6) {
          					inner +=    '<option value="'+result[i].parentidx+'">Raw material</option>';
          				} else if(result[i].parentidx == 7) {
          					inner +=    '<option value="'+result[i].parentidx+'">IT</option>';
          				} else if(result[i].parentidx == 8){
          					inner +=    '<option value="'+result[i].parentidx+'">Circulation</option>';
          				} 
        		    	...
        		    	inner += '<tr>';
        		    	inner += 	'<th>계열사명</th>';
        		    	inner += 		'<td><input type="text" name="sub_name2" value="'+nullChk(result[i].cate_name_en)+'"/></td>';
        		    	inner +=  '</tr>';
        		    	
        		    	inner +=   '<tr>';
        		    	inner +=     '<th>계열사 설명</th>';
        		    	inner +=          '<td><input type="text" name="sub_dis" value="'+nullChk(result[i].cate_dis_en)+'"/></td>';
        		    	inner +=   '</tr>';
		...
		$.ajax({
			...
			success : function(data) 
			{
    				...
    				for(var i = 0; i < result.length; i++) {
    				     ...
    				     inner +=   	  '<option value="'+result[i].idx+'">'+result[i].cate_brand_en+'</option>';
...
// 		formData.append("cate_logo[]", cate_logo[1]);
		...
		$.ajax({
			type : "POST", 
			url : "/aDmin/cate/cate_submit_en.php",
...
// 		formData.append("cate_logo[]", cate_logo[1]);
		...
		$.ajax({
			type : "POST", 
			url : "/aDmin/cate/cate_sub_submit_en.php",
			enctype: 'multipart/form-data',
			processData: false,
            contentType: false,
			async : false,

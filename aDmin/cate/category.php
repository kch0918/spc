...
<script>
			...
			success : function(data) 
			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        		    	...
        		    	inner +=  	    '<input type="text" data-idx="'+result[i].idx+'" name="cate_brand_kr" value="'+result[i].cate_brand_kr+'">';
			...
			success : function(data) 
			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        		    	...
        		    	inner +=  	    '<input type="text" data-sub-idx="'+result[i].idx+'" name="cate_brand_kr" value="'+result[i].cate_brand_kr+'">';
    ...
    function editcate_proc() {
    	...
    	cate_brand_kr = $('input[data-idx='+chkidx+']').val(); 
    	
    	console.log(chkidx);
    	console.log(cate_brand_kr);
    	...
    	$.ajax({
    			type : "POST", 
    			url : "/aDmin/cate/editcate_proc.php",
    			dataType : "text",
    			async : false,
    			data : 
    			{
    ...
    function editcate_sub_proc() {
    	...
    	cate_brand_kr = $('input[data-sub-idx='+chkidx+']').val(); 
    	...
    	$.ajax({
    			type : "POST", 
    			url : "/aDmin/cate/editcate_sub_proc.php",
    			dataType : "text",
    			async : false,
    			data : 
    			{
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
					if(result[i].expo_yn == "Y")
					{
        		    	...
        		    	inner +=            '<td><input type="text" data-idx2='+result[i].idx+' name="brand_name" value="'+result[i].cate_name+'"/></td>';
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
    			 	if(result[i].expo_yn == "Y")
    				{
            		    	...
            		    	inner +=            '<td><input type="text" data-sub-idx2='+result[i].idx+' name="brand_name" value="'+result[i].cate_name+'"/></td>';
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
						...
						if(result[i].parentidx == 1) {
                       		inner +=    '<option value="'+result[i].parentidx+'">베이커리/디저트</option>';
						} else if(result[i].parentidx == 2) {
							inner +=    '<option value="'+result[i].parentidx+'">외식/다이닝</option>';
						} else if(result[i].parentidx == 3) {
							inner +=    '<option value="'+result[i].parentidx+'">커피/음료</option>';
						} else if(result[i].parentidx == 4){
							inner +=    '<option value="'+result[i].parentidx+'">유통/서비스</option>';
						} 
        		    	...
        		    	inner += '<tr>';
        		    	inner += 	'<th>브랜드명</th>';
        		    	inner += 		'<td><input type="text" name="brand_name2" value="'+nullChk(result[i].cate_name)+'"/></td>';
        		    	inner +=  '</tr>';
        		    	
        		    	inner +=   '<tr>';
        		    	inner +=     '<th>브랜드 설명</th>';
        		    	inner +=          '<td><input type="text" name="brand_dis" value="'+nullChk(result[i].cate_dis)+'"/></td>';
        		    	inner +=   '</tr>';
				...
				for(var i = 0; i < result.length; i++) {
			 	var inner = "";
        				...
        				if(result[i].parentidx == 5) {
                       		inner +=    '<option value="'+result[i].parentidx+'">식품</option>';
        				} else if(result[i].parentidx == 6) {
        					inner +=    '<option value="'+result[i].parentidx+'">원료</option>';
        				} else if(result[i].parentidx == 7) {
        					inner +=    '<option value="'+result[i].parentidx+'">IT/서비스</option>';
        				} else if(result[i].parentidx == 8){
        					inner +=    '<option value="'+result[i].parentidx+'">유통</option>';
        				} 
        		    	...
        		    	inner += '<tr>';
        		    	inner += 	'<th>계열사명</th>';
        		    	inner += 		'<td><input type="text" name="sub_name2" value="'+nullChk(result[i].cate_name)+'"/></td>';
        		    	inner +=  '</tr>';
        		    	
        		    	inner +=   '<tr>';
        		    	inner +=     '<th>계열사 설명</th>';
        		    	inner +=          '<td><input type="text" name="sub_dis" value="'+nullChk(result[i].cate_dis)+'"/></td>';
        		    	inner +=   '</tr>';
		...
		$.ajax({
			...
			success : function(data) 
			{
    				...
    				for(var i = 0; i < result.length; i++) {
    				     ...
    				     inner +=   	  '<option value="'+result[i].idx+'">'+result[i].cate_brand_kr+'</option>';
...
// 		formData.append("cate_logo[]", cate_logo[1]);
		...
		$.ajax({
			type : "POST", 
			url : "/aDmin/cate/cate_submit.php",
...
// 		formData.append("cate_logo[]", cate_logo[1]);
		...
		$.ajax({
			type : "POST", 
			url : "/aDmin/cate/cate_sub_submit.php",
			enctype: 'multipart/form-data',
			processData: false,
            contentType: false,
			async : false,

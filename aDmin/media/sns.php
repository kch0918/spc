...
$query = "SELECT *,DATE_FORMAT(submit_date, '%Y-%m-%d') as submit_date2 FROM SPC_SNS where 1";
...
function changeFunc() {
	 ...
	 location.href = "/aDmin/media/sns.php?order_by="+selectedValue;
...
function goSns(idx)
{
	location.href="/aDmin/media/sns_edit.php?idx="+idx;
}
...
function goDelete(idx)
{
	if(confirm("해당 게시물을 삭제하시겠습니까?"))
    {
	   ...
	   $.ajax({
   		type: "post",
   		url: "/aDmin/media/sns_delete_proc.php",
   		dataType:"text",
   		data : 
   		{
       		idx : idx
...
<div id="container" class="report magazine">
	<div class="search">
		<p>SNS 컨텐츠 리스트<strong>생성된 게시글을 관리합니다.</strong></p>
		<input type="button" class="enroll" value="등록" onclick="location.href='/aDmin/media/sns_write.php'">
		<form id="fncForm" name="fncForm" method="post" action="sns.php">
                        ...
                        for ($i=0; $i<sql_count($result); $i++)
                        {
                            ...
                            ?>
            					<tr>
    								...
    								if($row['thumb'] != null) {
    								?>
        								<td><img src="/aDmin/file/<?php echo $row['thumb']?>"></td>
            						<?php 
    								} else {
    								?>
    									<td><img src="/aDmin/img/youtube-logo01.png"></td>
    								<?php 
    								}
            						?>
            						...
            						<?php if($row['expo_yn'] == "Y") {
            						    ...
            						    <?php 
                						  ...
                						  ?>
            						<td><a class="edit" onclick="goSns(<?php echo $row['idx']?>)">수정</a></td>

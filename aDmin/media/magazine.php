...
// if(isset($_REQUEST['datepicker1'])){
//     $date_picker1 = $_REQUEST['datepicker1'];
// }else{
//     $date_picker1 = "";
// }

$query = "SELECT *,DATE_FORMAT(submit_date, '%Y-%m-%d') as submit_date2 FROM SPC_MAGAZINE where 1";
...
if(isset($_POST['language']) && $_POST['language'] != null && $_POST['language'] != "")
{
    ...
    $query .= " and language in ( {$language} )";
}
...
function changeFunc() {
	 ...
	 location.href = "/aDmin/media/magazine.php?order_by="+selectedValue;
...
// 조회
function goMagazine(idx)
{
	location.href="/aDmin/media/magazine_edit.php?idx="+idx;
}
...
function goDelete(idx)
{
	if(confirm("해당 게시물을 삭제하시겠습니까?"))
    {
	   ...
	   $.ajax({
   		type: "post",
   		url: "/aDmin/media/magazine_delete_proc.php",
   		dataType:"text",
   		data : 
   		{
       		idx : idx
...
<div id="container" class="report magazine">
	<div class="search">
		<p>SPC 매거진<strong>생성된 게시글을 관리합니다.</strong></p>
		<input type="button" class="enroll" value="등록" onclick="location.href='/aDmin/media/magazine_write.php'">
		<form id="fncForm" name="fncForm" method="post" action="magazine.php">
	...
	<table class="table_tit magazine_tb">
		<tr>
        	<th>NO.</th>
        	<th>언어</th>
        	<th>발행호</th>
		...
		<table class="tablecont magazine_tb">
				<?php
                        $s_point = ($page-1) * $list;
            						...
            						<?php if($row['expo_yn'] == "Y") {
            						 ...
            						 <?php 
            						    ...
            						    <?php 
                						  ...
                						  ?>
            						<td><a class="edit" onclick="goMagazine(<?php echo $row['idx']?>)">수정</a></td>

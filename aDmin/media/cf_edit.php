...
$query = "select *,DATE_FORMAT(submit_date, '%Y.%m.%d') as submit_date2 from SPC_CF where idx = {$_REQUEST['idx']}";
	...
	if(validationFlag == "Y")
	{
		$("#fncForm").ajaxSubmit({
			...
			success: function(data)
			{
	    		...
	    		{
	    			alert("저장되었습니다.");
	    			location.href="/aDmin/media/cf.php";
	    		}
	    		else
	    		{
	    			alert(result.msg);
...
<div id="container" class="news write">
	<div class="search">
		<p>CF 컨텐츠 등록<strong>생성된 컨텐츠를 수정하고 관리합니다.</strong></p>		
	</div>
	<form id="fncForm"  method="post" action="cf_edit_proc.php" enctype="multipart/form-data" onSubmit="return false;">	
	<input type="hidden" name="idx" value="<?php echo $row["idx"]?>">
    	<div class="write_cont_box">
    		<div class="write_cont">
    			<div>
                    <div class="write_li_tit def-select">
                        <p class="write_cont_tit">구분</p>
                        <select class="seacrh_sel" name="type">
                            <option value="sort" disabled><?php echo $row['type']?></option>
                            <option>삼립</option>
                            <option>파리바게뜨</option>
                    ...
                    <div class="write_li_tit">	
                        ...
                        <div class="date_box">
                        	<input type="text" id="start_date" name="start_date" class="search_date notempty" data-name="등록일" placeholder="날짜를 입력해주세요" value="<?php echo $row['submit_date']?>">
                        </div>	
                	</div>		
    				<div class="write_li_tit">
                    ...
                    <div class="write_li_file">
                        <p class="write_cont_tit ver_middle">URL</p>
                        <input type="text" name="url" class="file_area notempty"  placeholder="URL 입력" data-name="URL" value="<?php echo $row['url']?>">
                    </div>
    				<div class="write_li_file">
		...
		<div class="write_bt">
			<ul>
				<li><a href="/aDmin/media/cf.php" class="bt_cont">목록</a></li>
